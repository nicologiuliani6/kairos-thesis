...
\medskip
\noindent
\textbf{Kairos} sta in quel territorio. Al nucleo di Janus (assegnamenti
invertibili, condizionali a doppia guardia, cicli \texttt{from\,\dots\,until},
procedure invocabili con \texttt{call} e \texttt{uncall}) aggiunge il
parallelismo esplicito \texttt{par\,\dots\,and\,\dots\,rap}, la comunicazione
sincrona su canali tipizzati \texttt{ssend}/\texttt{srecv}, uno stack come
struttura dati primitiva e le transazioni \texttt{try\,\dots\,rollback\,\dots\,yrt}.
...
c ::={}& \mathtt{skip} && \text{(comando vuoto)}\\
   \mid{}& v \mathbin{+\!\!=} e \mid v \mathbin{-\!\!=} e \mid v \mathbin{\hat{}\!=} e \mid v \mathbin{<\!=\!>} v && \text{(assegnamenti reversibili)}\\
...
(\S\ref{sec:rev}). Un programma ben formato soddisfa inoltre due vincoli: su
...
le variabili sono di tre tipi: \texttt{int} (interi), \texttt{stack} (sequenze di
interi) e \texttt{channel} (canali: nomi di sincronizzazione tra rami
paralleli). Un canale \textbf{non} contiene valori: è un punto di
...
\[
\sigma : \mathit{Var} \rightharpoonup \mathbb{Z} \cup \mathbb{Z}^{*} \cup \mathit{Chan},
\]
...
\textbf{esattamente due} rami sul canale (\S\ref{sec:chan}) e la sequenza di
comandi di ciascun ramo fissata dal testo del programma, l'accoppiamento è
...
Gli algoritmi di compressione lavorano per indice, e Janus ha gli array. Kairos
ha \texttt{int}, \texttt{stack} e \texttt{channel}, e uno stack si tocca solo in
cima: l'accesso per indice va costruito, e resta dentro il linguaggio.

\begin{lstlisting}
procedure leggi(stack s, stack aux, int i, int r)
    call sposta(s, aux, i)      // porta i elementi sull'appoggio
    local int v = 0
        pop(v, s)
        r += v
        push(v, s)
    delocal int v = 0
    uncall sposta(s, aux, i)    // e li rimette a posto
\end{lstlisting}

\noindent
Rimettere a posto non è codice nuovo: è \texttt{uncall} dello spostamento,
perché \texttt{push} e \texttt{pop} sono già una coppia inversa esatta. Il costo
è però $O(i)$ per accesso, cioè $O(n)$ nel caso peggiore, e si paga su ogni
lettura del blocco: è la differenza che separa la nostra complessità dalla loro.
...
indice & \texttt{3}\\
ricostruzione da (colonna, indice) & \texttt{[2, 1, 3, 1, 3, 1]}\\
\hline
\end{tabular}
\end{center}
...
lo percorre a partire dall'indice. Verificata anche su blocchi di dodici simboli
e tre blocchi concorrenti.

\paragraph{Costo.} L'esponente misurato passa da $3{,}29$ a $3{,}57$ fra $n=6$ e
$n=12$, in avvicinamento a $n^4$, contro gli $n^3$ del lavoro di riferimento. Il
fattore in più è esattamente l'accesso per indice emulato sopra gli stack, e la
misura lo conferma invece di limitarsi a prevederlo.

\paragraph{Concorrenza.} Lo stesso calcolo con i blocchi in sequenza e con i
blocchi come rami di un \texttt{par}, su una macchina a ventiquattro nuclei:

\begin{center}
\begin{tabular}{llll}
\hline
blocchi & sequenziale (s) & concorrente (s) & guadagno\\
\hline
2 & 4{,}44  & 3{,}53 & 1{,}26$\times$\\
4 & 8{,}84  & 4{,}28 & 2{,}07$\times$\\
6 & 13{,}30 & 6{,}42 & 2{,}07$\times$\\
8 & 17{,}51 & 7{,}52 & 2{,}33$\times$\\
\hline
\end{tabular}
\end{center}

\noindent
Il guadagno cresce col numero di rami, ma resta lontano dal numero di blocchi:
il tempo non si divide per otto. Il collo di bottiglia non è nel linguaggio, che
per costruzione non fa condividere nulla ai rami, ma nell'esecutore, dove
strutture globali serializzano operazioni che i rami compiono in modo
indipendente (\cref{chap:interprete}). Resta poi un limite proprio della misura:
il raccoglitore è un ramo solo, e riceve dai canali uno per volta.
...
costo $n^4$; con accesso per indice nativo si arriverebbe alla complessità del
lavoro di riferimento. E la proprietà \emph{clean} nel senso stretto, cioè
...
% ======================================================================
\chapter{Implementazione: l'interprete}
...
punto giusto. La seconda è che l'inversione non ha bisogno di una
rappresentazione separata: ripercorre lo stesso testo al contrario.
...
@3   JMPF FROM_ERR_5         // falsa: salta tutto il ciclo
@3   JMP FROM_START_5        // vera: entra saltando il secondo corpo
@3   LABEL FROM_BACK_5
...
\emph{back-edge}. Le etichette non sono decorative: l'inversione non ha l'albero
sintattico e ricostruisce la struttura del ciclo proprio dai loro nomi.
...
struttura associata; più le mappe che traducono i nomi in indici. Una
configurazione $\langle \sigma, c\rangle$ è la coppia frame corrente e puntatore
alla riga di bytecode.

La traduzione nome-indice è una scansione lineare con confronto sul primo
carattere prima di quello completo: per il numero di nomi di un frame batte una
tabella hash, che pagherebbe l'hashing per un guadagno asintotico che non si
materializza. Le mappe sono però dimensionate staticamente, e questo domina
l'occupazione: un frame pesa oltre due megabyte quasi indipendentemente da
quante variabili contenga, ed è il costo che si moltiplica per livello quando la
ricorsione richiede istanze separate dello stesso frame.

\paragraph{Ciclo principale.} L'interprete legge una riga, ne estrae l'opcode e
lo confronta con una catena di nomi:
...
reversibilità del linguaggio si paga da sola: l'inverso di un opcode è
\emph{un altro opcode}, e la tavola degli inversi si scrive in quattro righe.
Ciò che costa è ricostruire il cammino, non ripristinare i valori: quali rami
sono stati presi, dove comincia e finisce un ciclo, quale iterazione si sta
disfacendo.
...
diventano solo se non condividono nulla \emph{nemmeno nell'esecutore}. Tre
strutture stanno sul cammino di ogni ramo e vanno tenute per thread o toccate
con operazioni atomiche: lo stato della tokenizzazione, che è per thread; il
rilevatore di scritture concorrenti su una cella intera, che vive nella cella e
non in un lock unico; e l'indice che associa a ciascun ramo la propria copia del
frame di una procedura, che si consulta senza esclusione perché è a sola
aggiunta e pubblica ogni voce solo quando è completa. Sono i punti in cui una
struttura globale, anche solo di servizio, annullerebbe il parallelismo che il
linguaggio garantisce.

\medskip
\noindent
Un costo resta, ed è la copia del bytecode: ogni ramo ne riceve una completa,
perché la tokenizzazione scrive terminatori dentro il testo e due thread sullo
stesso buffer si corromperebbero. Per i programmi di \cref{chap:programmare}
sono diciotto kilobyte per ramo, poche centinaia di microsecondi su esecuzioni
di secondi: si toglierebbe copiando la sola riga in lavorazione, ma non è lì che
si gioca il parallelismo. Il guadagno misurato resta infatti sotto il numero di
rami, e la distanza non ha una spiegazione già pronta.
...
trasformata no: costa $O(n^3)$ già con gli array, e qui $O(n^4)$ perché
l'accesso per indice è emulato sopra gli stack. Due cose lo colmerebbero, ed è
la direzione che indicheremmo per prima. La prima è un tipo array nel
linguaggio, con le sue regole nella semantica: toglierebbe il fattore $n$ e
allineerebbe il costo a quello del lavoro di riferimento. La seconda è un
ordinamento delle rotazioni migliore di quello ingenuo, che è il punto su cui
quel lavoro stesso dichiara che i miglioramenti restano necessari. Finché
entrambe mancano, i blocchi restano corti, e con blocchi corti la trasformata
non ha il materiale su cui produrre l'effetto per cui esiste. Manca inoltre la
...
\paragraph{L'implementazione.} Il guadagno del calcolo concorrente cresce con il
numero di rami ma resta sotto di esso, e la distanza non ha una spiegazione già
pronta: individuare gli altri punti in cui l'esecutore condivide ciò che il
linguaggio tiene separato è lavoro di misura prima che di progetto. Sul fronte
della memoria, un frame costa oltre due megabyte quasi indipendentemente da
quante variabili contenga, perché le tabelle dei nomi sono dimensionate
staticamente; dimensionarle sul programma è l'intervento singolo che cambierebbe
di più il profilo d'occupazione, e diventa vincolante appena la ricorsione
richiede istanze separate dello stesso frame.
